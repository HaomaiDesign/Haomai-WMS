					...
					$sql0 = "SELECT COUNT(*) AS rowNum FROM(SELECT idDelivery, datetime, SUM(quantity) AS quantities, SUM(items) AS items, COUNT(idOrder) AS orders
											FROM 
												(SELECT a.id AS idDelivery, a.datetime, SUM(c.quantity) AS quantity, b.id AS idOrder, COUNT(c.unitBarcode) as items FROM delivery AS a 
												LEFT JOIN orders AS b ON a.id=b.deliveryId 
												LEFT JOIN orderDetails AS c ON b.id=c.orderId 
												GROUP BY idOrder) AS resultTable 
											GROUP BY idDelivery
											ORDER BY idDelivery DESC) AS resultTable2;";  
							  ...
							  <div class="table-responsive">
								<table id="table" class="table table-hover table-outline table-vcenter text-truncate card-table">
								  ...
								  <tbody>
									...
									// $sql = "SELECT id, datetime, SUM(quantity) AS quantities, COUNT(unitbarcode) AS products, orders 
									// 		FROM 
									// 			(SELECT a.id, a.datetime, SUM(c.quantity) AS quantity, COUNT(b.id) AS orders, c.unitBarcode FROM delivery AS a 
									// 			LEFT JOIN orders AS b ON a.id=b.deliveryId 
									// 			LEFT JOIN orderDetails AS c ON b.id=c.orderId 
									// 			GROUP BY a.id, c.unitBarcode) as newTable 
									// 		GROUP BY id 
									// 		ORDER BY id DESC 
									//  		LIMIT $start, $limit;";
									
									
									$sql = "SELECT idDelivery, datetime, SUM(quantity) AS quantities, SUM(items) AS items, COUNT(idOrder) AS orders
											FROM 
												(SELECT a.id AS idDelivery, a.datetime, SUM(c.quantity) AS quantity, b.id AS idOrder, COUNT(c.unitBarcode) as items FROM delivery AS a 
												LEFT JOIN orders AS b ON a.id=b.deliveryId 
												LEFT JOIN orderDetails AS c ON b.id=c.orderId 
									...
									<tr>
									  ...
									  <td>
										<div><?php echo str_pad($row['idDelivery'], 8, "0", STR_PAD_LEFT);?></div>
									  </td>
									  ...
									  <td class="text-center">
										<div><?php echo number_format($row['items'],2,",","."); ?></div>
									  </td>
									  <td class="text-center">
									  ...
									  <td class="text-center">
										  <div>
											<a href="printRoute.php?deliveryId=<?php echo $row['idDelivery']; ?>&businessId=<?php echo $_SESSION['user']['businessId']; ?>&date=<?php echo date('d-m-Y', strtotime($row['datetime'])); ?>" target="_blank"><i class="dropdown-icon fe fe-printer"></i></a>
											<a href="deliveryList.php?tableStatus=view&target=out&businessId=<?php echo $_SESSION['user']['businessId']; ?>&deliveryId=<?php echo $row['idDelivery']; ?>&page=1"><i class="dropdown-icon fe fe-eye"></i></a>
											<a onclick="removeDelivery('<?php echo $row['idDelivery'];?>')"><i class="dropdown-icon fe fe-x"></i></a>
										  </div>
									  </td>
									</tr>
									<?php }; ?>
