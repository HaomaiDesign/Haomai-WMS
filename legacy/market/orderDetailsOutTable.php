		...
		<div class="col-lg-12">
			<form class="card" action="" autocomplete="off" method="post" enctype="multipart/form-data">
				...
				<div class="card-header">
					...
					<a style="margin-left:5%; cursor:pointer;" onclick="clientInfoVisibility();"><i
							class="fe fe-eye fa-lg"></i></a>
				...
				<?php if ($_GET['target'] == 'out') { ?>
						...
						<div class="row">
							...
							<!-- <div class="col-sm-3 col-md-3">
											<div class="form-group">
												...
												<input type="text" name="whatsapp" id="whatsapp" onchange="update('<?php echo $_SESSION['form']['table']; ?>','<?php echo $_SESSION['form']['server']['id']; ?>','whatsapp','1')" class="form-control" value="<?php if (isset($_SESSION['form']['server']['whatsapp']))
											...
											<div class="form-group">
												...
												<input type="text" name="wechat" id="wechat" onchange="update('<?php echo $_SESSION['form']['table']; ?>','<?php echo $_SESSION['form']['server']['id']; ?>','wechat','1')" class="form-control" value="<?php if (isset($_SESSION['form']['server']['wechat']))
							...
							<!-- <div class="col-sm-3 col-md-3">
											<div class="form-group">
												<label class="form-label"><?php echo $_SESSION['language']['Postal Code']; ?></label>
												<input type="text" name="postalCode" id="postalCode" onchange="update('<?php echo $_SESSION['form']['table']; ?>','<?php echo $_SESSION['form']['server']['id']; ?>','postalCode','1')" class="form-control" value="<?php if (isset($_SESSION['form']['server']['postalCode']))
							...
							<!-- <div class="col-sm-4 col-md-4">
											<div class="form-group">
												<label class="form-label"><?php echo $_SESSION['language']['Location']; ?></label>
												<input type="text" name="location" id="location" onchange="update('<?php echo $_SESSION['form']['table']; ?>','<?php echo $_SESSION['form']['server']['id']; ?>','location','1')" class="form-control" value="<?php if (isset($_SESSION['form']['server']['location']))
											...
											<div class="form-group">
												<label class="form-label"><?php echo $_SESSION['language']['Province / State']; ?></label>
												<input type="text" name="province" id="province" onchange="update('<?php echo $_SESSION['form']['table']; ?>','<?php echo $_SESSION['form']['server']['id']; ?>','province','1')" class="form-control" value="<?php if (isset($_SESSION['form']['server']['province']))
							...
							<!-- <div class="col-sm-12 col-md-12">
											<div class="form-group">
												...
												<input type="text" name="description" id="description" onchange="update('<?php echo $_SESSION['form']['table']; ?>','<?php echo $_SESSION['form']['server']['id']; ?>','description','1')" class="form-control" value="<?php if (isset($_SESSION['form']['server']['description']))
					...
					<div id="clientDataCollapsed" class="card-body">
						<h5><?php if (isset($_SESSION['form']['server']['businessName']))
							echo $_SESSION['language']['Customer'] . " : " . $_SESSION['form']['server']['businessName']; ?>
						</h5>
					</div>
				<?php } ?>
			</form>
			...
			<div class="card">
				...
				<div class="card-header">
					...
					<div class="col-9">
						<form onsubmit="sendBarcodeToUpdate()" autocomplete="off" method="get">
							...
							<input type="hidden" id="businessIdInput" name="businessId" class="form-control"
								value="<?= $_SESSION['user']['businessId']; ?>">
							...
							<div class="row">
								<div class="col-">
									<label id="unitBarcodeTitleId"
										class="form-label"><?= $_SESSION['language']['Pack Barcode']; ?></label>
									<!-- Esto en realidad esta cargado en la columna de Unit Barcode -->
								...
								<div class="col my-auto ml-4">
									<a href="printSaleOrderPage.php?target=out&businessId=<?= $_GET['businessId']; ?>&userId=<?= $_GET['userId']; ?>&date=<?= date('d-m-Y', strtotime($_GET['date'])); ?>&requestId=<?= $_GET['requestId']; ?>&orderId=<?= $_GET['id']; ?>"
					...
					<div class="col-3 ml-5">
						<button type="button" onclick="removeProductByOrderId(<?= $_GET['id'] ?>)" class="btn btn-dark"><i
								class="fas fa-trash mr-2"></i><?= $_SESSION['language']['Clear all products'] ?></button>
					</div>
					<!--
								<div class="col-6">
					...
					<table id="table" class="table table-outline table-vcenter text-nowrap card-table">
						<thead>
							<tr>
								...
								<!-- <th style="width:25%;font-weight:bold;"> <?php echo $_SESSION['language']['Product Name']; ?></th> -->
								<th class="text-center" style="width:5%;font-weight:bold;"><i class="fas fa-sort"></i>
									<?php echo $_SESSION['language']['Due Date']; ?></th>
								<th class="text-center" style="width:5%;font-weight:bold;">
									<?php echo $_SESSION['language']['Pack']; ?>
								</th>
							...
							if ($stmt) {
								while ($row = mysqli_fetch_array($stmt, MYSQLI_ASSOC)) {
									...
									<tr id="productRowDetailsId-<?= $row['id']; ?>">
										...
										<!-- <td>
											<div><?php if ($row['quantity'] == 0)
												echo "<strike>" . $row['productName2'] . "</strike>";
											else
												echo $row['productName2']; ?></div>
										</td> -->
										<td>
											<div style="text-align:center">
												...
												<?php } else { ?>
													<button type="button" id="modifyDates"
														onclick="openModalDates('<?= $row['unitBarcode']; ?>', 'out', <?= $row['orderId']; ?>)"
	...
	<div class="modal-dialog modal-lg">
		...
		<div class="modal-content">
			...
			<div class="modal-footer">
				<button id="closeModal" type="button" onclick="location.reload()" class="btn btn-default"
...
<script>
	...
	function lowStockNotification(unitbarcode) {
		console.log('entre bien', unitbarcode);
		// if(isset($_SESSION['outProds'])) {
		// 	unset($_SESSION['outProds']);
		// }
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (data["type"] == "warning") {
					toastr.options.positionClass = "toast-down-left";
					toastr.warning("El producto " + data["product"] + " tiene stock bajo: " + data["stock"]);
					// No funciona el notification
					// notification(data.type,"El producto " + data["productName"] + " tiene stock bajo: " + data["stock"])
				}
			},
			error: function (data) {
				toastr.options.positionClass = "toast-down-left";
				toastr.error("No se pudo verificar el estado del stock");
			}
		});
	}
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (data["type"] == "success") {
					confirmationSound();
					notification(data.type, data.message);
					lowStockNotification(unitbarcode)
	...
	function changeLoteQty(loteId, orderId, flagInOut, dueDate, stock, available, oldQty, sku, name, name2, unitBarcode, image, unit, pack, capacity) {
		let loteQty = document.getElementById("loteQty" + loteId).value;

		if (loteQty == 1) {
			document.getElementById("loteQty" + loteId).setAttribute("disabled", "disabled");
		}
		...
		$.ajax({
			...
			success: function (data) {
				...
				toastr.options.positionClass = "toast-top-left";
				console.log(data); // Inspect this in your console
				document.getElementById("loteQty" + loteId).removeAttribute("disabled");
			},
			error: function (data) {
				toastr.options.positionClass = "toast-top-left";
				toastr.error("No se pudo actualizar");
